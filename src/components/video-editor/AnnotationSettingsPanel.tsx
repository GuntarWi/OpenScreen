...
interface AnnotationSettingsPanelProps {
  ...
  onEffectChange?: (patch: Partial<AnnotationRegion>) => void;
  onEmojiChange?: (emoji: TelegramEmoji) => void;
  onLayerChange?: (layer: AnnotationRegion['layer']) => void;
...
export function AnnotationSettingsPanel({
  ...
  const handleEmojiSelect = (emoji: TelegramEmoji) => {
    onTypeChange('emoji');
    if (onEmojiChange) {
      onEmojiChange(emoji);
    } else {
      onContentChange(emoji.src);
    }
  };

  const activeLayer = annotation.layer || 'foreground';

  const emitTimingChange = (nextStart: number, nextEnd: number) => {
    if (onTimingChange) {
      const validStart = Math.max(0, Math.min(nextStart, nextEnd));
      const validEnd = Math.max(validStart + 1, nextEnd);
      onTimingChange(validStart, validEnd);
    }
  };
      ...
      <div className="mb-6">
        ...
        <Tabs value={annotation.type} onValueChange={(value) => onTypeChange(value as AnnotationType)} className="mb-6">
          <TabsList className="mb-4 bg-white/5 border border-white/5 p-1 w-full grid grid-cols-4 h-auto rounded-xl">
            ...
            <TabsTrigger value="emoji" className="data-[state=active]:bg-[#34B27B] data-[state=active]:text-white text-slate-400 py-2 rounded-lg transition-all gap-2">
          ...
          {onTimingChange && (
            <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-xs font-medium text-slate-200">Timing</div>
                  <p className="text-[11px] text-slate-500">Control when this appears.</p>
                </div>
                <span className="text-[11px] text-slate-400 font-mono">
                  {((endMs - startMs) / 1000).toFixed(2)}s
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[11px] text-slate-400 mb-1 block">Start (s)</label>
                  <input
                    type="number"
                    min={0}
                    step={0.1}
                    ...
                    onChange={(e) => {
                      ...
                      setStartMs(value);
                      emitTimingChange(value, endMs);
                    }}
                    className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-slate-200 text-sm"
                  />
                </div>
                <div>
                  <label className="text-[11px] text-slate-400 mb-1 block">End (s)</label>
                  <input
                    type="number"
                    min={0}
                    step={0.1}
                    ...
                    onChange={(e) => {
                      const value = Number(e.target.value) * 1000;
                      setEndMs(value);
                      emitTimingChange(startMs, value);
                    }}
                    className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-slate-200 text-sm"
                  />
                </div>
          ...
          <TabsContent value="emoji" className="mt-0 space-y-4">
            {onEffectChange && (
              <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-medium text-slate-200">Enter / Exit</div>
                  <span className="text-[11px] text-slate-500">Transitions</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[11px] text-slate-400 mb-1 block">Enter</label>
                    <Select
                      value={annotation.enterEffect || 'none'}
                      onValueChange={(value) => onEffectChange({ enterEffect: value as AnnotationRegion['enterEffect'] })}
                    >
                      <SelectTrigger className="w-full bg-white/5 border-white/10 text-slate-200 h-9 text-xs">
                  ...
                  <div>
                    <label className="text-[11px] text-slate-400 mb-1 block">Exit</label>
                    <Select
                      value={annotation.exitEffect || 'none'}
                      onValueChange={(value) => onEffectChange({ exitEffect: value as AnnotationRegion['exitEffect'] })}
                    >
                      <SelectTrigger className="w-full bg-white/5 border-white/10 text-slate-200 h-9 text-xs">
                        <SelectValue placeholder="Choose" />
                      </SelectTrigger>
                      <SelectContent className="bg-[#1a1a1c] border-white/10 text-slate-200">
                        ...
                        <SelectItem value="pop">Pop out</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[11px] text-slate-400 mb-1 block">Fade In (ms)</label>
                    <input
                      type="number"
                      min={0}
                      max={2000}
                      step={20}
                      value={annotation.fadeInMs ?? 240}
                      onChange={(e) => onEffectChange({ fadeInMs: Number(e.target.value) })}
                      className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-slate-200 text-sm"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] text-slate-400 mb-1 block">Fade Out (ms)</label>
                    <input
                      type="number"
                      min={0}
                      max={2000}
                      step={20}
                      value={annotation.fadeOutMs ?? 240}
                      onChange={(e) => onEffectChange({ fadeOutMs: Number(e.target.value) })}
                      className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-slate-200 text-sm"
                    />
                  </div>
                </div>
            ...
            <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[11px] text-slate-500">Layering</span>
              </div>
              <Select
                value={activeLayer}
                onValueChange={(value) => onLayerChange?.(value as AnnotationRegion['layer'])}
              ...
              >
                ...
                <SelectContent className="bg-[#1a1a1c] border-white/10 text-slate-200">
                  <SelectItem value="foreground">On top of screen</SelectItem>
                  <SelectItem value="midground">Between wallpaper & screen</SelectItem>
                </SelectContent>
              </Select>
              <p className="text-[11px] text-slate-500">
                Midground keeps the emoji behind the recording but above the wallpaper.
              </p>
            </div>
          </TabsContent>
